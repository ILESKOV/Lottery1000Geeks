...
describe("Lottery", function () {
  ...
  Lottery = await lottery.deploy(1, PriceFeedMock.address, VrfCoordinatorV2Mock.address, keyhash);
  }) 
  describe("logic tests", function () {
    ...
    describe("Before lottery started", function (){
      ...
      it('should not allow end lottery that was not started yet', async function(){
        ...
        ).to.be.revertedWith("Can't end lottery yet");
      });
      ...
      it('should not allow participate while lottery is closed', async function(){
        ...
        )).to.be.revertedWith("The lottery is closed. Wait until the next lottery");
      });
      ...
      it('corrects get the participation fee', async () => {
        ...
        expect(await participationFee.toString()).to.equal(ethers.utils.parseEther("0.05"));
      });
    })
    ...
    describe("After lottery started", function (){
      ...
      it('disallow participation without enough money', async () => {
        await expect(Lottery.connect(player1).participate({ value: ethers.utils.parseEther("0.049")})
        ).to.be.revertedWith("Not Enough ETH to participate!");
      }); 
      
      it('checks lottery balance after lottery started', async () => {
        let participationFee = await Lottery.getParticipationFee()
        await Lottery.connect(player1).participate({ value: ethers.utils.parseEther("0.05")});
        await Lottery.connect(player2).participate({ value: ethers.utils.parseEther("0.05")});
        await Lottery.connect(player3).participate({ value: ethers.utils.parseEther("0.05")});
        expect(await Lottery.getLotteryBalance() == participationFee * 3);
      });
    })
    ...
    describe("After selecting the winner", function (){
      ...
      beforeEach(async () => {
        ...
        await Lottery.connect(player3).participate({ value: ethers.utils.parseEther("0.05")});
        await Lottery.endLottery();
        // let balanceBefore = await player2.getBalance() / 1000000000000000000;
